...
declare -r G_USER_PACKAGES="auditd avahi-daemon bash-completion bc binutils cockpit cockpit-networkmanager curl debsecan dnsutils git gpiod inetutils-ping jq libsystemd-dev libzmq3-dev libzmq5 lm-sensors lsb-release network-manager-openvpn  nlohmann-json3-dev openvpn podman pciutils pkgconf python3-asteval python3-cryptography python3-dateutil python3-libgpiod python3-lxml python3-pip python3-psutil python3-serial python3-websocket python3-websockets python3-zmq screen sqlite3 stress-ng sudo sysstat systemtap-sdt-dev time tmux traceroute u-boot-tools uuid-runtime vim wget wireguard-tools zram-tools zstd"
