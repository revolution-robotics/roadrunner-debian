...
[Service]
ExecStart=
ExecStart=/usr/bin/bluealsa -c LC3plus --sbc-quality=xq+ --aac-afterburner --keep-alive=5 -p a2dp-source -p a2dp-sink -p hfp-ag -p hfp-hf -p hsp-ag -p hsp-hs --io-rt-priority=50
...
EOF
    ...
    install -m 0755 "${G_VENDOR_PATH}/resources/reverse-tunnel-server/bootstrap-reverse-tunnel-server" \
            "${ROOTFS_BASE}/usr/bin/bootstrap-reverse-tunnel-server"
