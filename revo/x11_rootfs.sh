...
EOF
    ...
    sed -i -e 's/\(^MAIN_LOCAL_DOMAINS=@:localhost\).*/\1:'"$MACHINE"'/' \
        -e 's/\(^ETC_MAILNAME\).*/\1='"$MACHINE"'/' \
        "${ROOTFS_BASE}/var/lib/exim4/config.autogenerated"
    ...
    # Install systemd {tmp,var-tmp,var-log}.mount.
    install -m 0644 "${G_VENDOR_PATH}/${MACHINE}/var-"{log,tmp}.mount \
            "${ROOTFS_BASE}/usr/share/systemd"
    ln -s '/usr/share/systemd/'{tmp,var-{log,tmp}}.mount \
       "${ROOTFS_BASE}/etc/systemd/system"

    # Regenerate SSH keys on first boot
    install -m 0644 "${G_VENDOR_PATH}/${MACHINE}/regenerate_ssh_host_keys.service" \
            "${ROOTFS_BASE}/lib/systemd/system"
    ln -s '../regenerate_ssh_host_keys.service' \
       "${ROOTFS_BASE}/lib/systemd/system/multi-user.target.wants"

    # Install NetworkManager autoshare dispatcher.
