...
usage ()
{
    ...
    echo
    echo " OPTIONS:"
    echo " -b <dart6ul|som6ul|mx7>  Board model (DART-6UL/VAR-SOM-6UL/REVO-ROADRUNNER-MX7) - optional, autodetected if not provided."
    ...
    echo " -m			REVO-ROADRUNNER-MX7 optional Cortex-M4 support; ignored in case of 6UL."
...
if [[ $STORAGE_DEV == "nand" ]] ; then
    ...
    if [[ $BOARD == *6ul ]] ; then
        ...
        KERNEL_DTB="imx7d-roadrunner-nand${VARSOMMX7_VARIANT}.dtb"
    fi
    ...
    if [[ $BOARD == *6ul ]] ; then
        ...
        block=mmcblk2
        KERNEL_DTBS="imx7d-roadrunner-emmc*.dtb"
